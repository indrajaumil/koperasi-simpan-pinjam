...
/**
 * This is the model class for table "m_user_online".
 *
 * The followings are the available columns in table 'm_user_online':
 * @property string $id
 ...
 */
class MUserOnline extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return MUserOnline the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			...
			array('password, activationKey, email', 'length', 'max'=>128),
			array('avatar', 'length', 'max'=>255),
			array('notifyType', 'length', 'max'=>9),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, username, password, activationKey, createtime, lastvisit, lastaction, lastpasswordchange, superuser, status, avatar, notifyType, email', 'safe', 'on'=>'search'),
		);
	}
